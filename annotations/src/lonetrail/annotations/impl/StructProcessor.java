...
import lonetrail.annotations.Annotations.StructField.*;
...
public class StructProcessor extends BaseProcessor {
    ...
    static String bitString(int offset, int size, int totalSize) {
        ...
        builder.append("0".repeat(Math.max(0, offset)));
        builder.append("1".repeat(Math.max(0, size)));
        builder.append("0".repeat(Math.max(0, totalSize - size - offset)));
    ...
    static String bitString(int size, int totalSize) {
        StringBuilder builder = new StringBuilder();
        builder.append("1".repeat(Math.max(0, size)));
        builder.append("0".repeat(Math.max(0, totalSize - size)));
        return "0b" + builder.reverse() + "L";
    }
    ...
    public void process(RoundEnvironment roundEnv) throws Exception {
        if (round == 1) { // Round 1: Register all struct informations
            ...
            for (TypeElement e : defs) {
                if (!simpleName(e).endsWith("Struct"))
                    throw new IllegalArgumentException(e + ": All @Struct types name must end with 'Struct;");
            ...
            for (VariableElement e : wrappers) {
                ...
                for (int i = left ? (vals.length - 1) : 0; left ? i >= 0 : i < vals.length; i += left ? -1 : 1) {
                    ...
                    if (!is(field, Modifier.PUBLIC) || is(field, Modifier.STATIC))
                        throw new IllegalArgumentException(type + "#" + fname + " must be an instance public field");
                    ...
                    if (defSize < size)
                        throw new IllegalArgumentException(kind + ": Size can't be greater than " + defSize + ": " + size);
                    if (kind == TypeKind.BOOLEAN && size != 1)
                        throw new IllegalArgumentException(kind + ": Size must be 1");
    ...
    public Set<String> getSupportedAnnotationTypes() {
        return Set.of(Struct.class.getCanonicalName(), StructWrap.class.getCanonicalName());
    }
    ...
    static class SInfo {
        ...
        SInfo(Class<?> clazz, int sized, FloatPacker packer) {
            type = clazz;
            size = sized;
            pack = packer;
        }
