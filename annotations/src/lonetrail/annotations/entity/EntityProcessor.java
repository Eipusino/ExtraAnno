package lonetrail.annotations.entity;
...
import lonetrail.annotations.Annotations.Resolve.*;
import lonetrail.annotations.*;
import lonetrail.annotations.util.*;
import lonetrail.annotations.util.TypeIOResolver.*;
...
public class EntityProcessor extends BaseProcessor {
    ...
    public void process(RoundEnvironment roundEnv) throws Exception {
        ...
        for (ExecutableElement e : (Set<ExecutableElement>) roundEnv.getElementsAnnotatedWith(Insert.class)) {
            if (!e.getParameters().isEmpty())
                throw new IllegalStateException("All @Insert methods must not have parameters");
        ...
        for (ExecutableElement e : (Set<ExecutableElement>) roundEnv.getElementsAnnotatedWith(Wrap.class)) {
            if (!e.getParameters().isEmpty())
                throw new IllegalStateException("All @Wrap methods must not have parameters");
            if (e.getReturnType().getKind() != BOOLEAN)
                throw new IllegalStateException("All @Wrap methods must have boolean return type");
                ...
                for (TypeElement comp : defComps) {
                    ...
                    for (VariableElement field : fields) {
                        ...
                        if (annotation(field, SyncField.class) != null && isSync) {
                            if (field.asType().getKind() != FLOAT)
                                throw new IllegalStateException("All SyncFields must be of type float");
            ...
            for (EntityDefinition def : definitions) {
                ...
                for (TypeElement comp : def.components) {
                    ...
                    for (ExecutableElement method : methods(inter)) {
                        ...
                        if (method.getReturnType().getKind() == VOID && !Seq.with(field.annotations).contains(f -> f.type.toString().equals("@lonetrail.annotations.Annotations.ReadOnly"))) {
    ...
    boolean append(MethodSpec.Builder mbuilder, Seq<TypeElement> defComps, Seq<ExecutableElement> values, Seq<ExecutableElement> inserts, Seq<ExecutableElement> wrappers, boolean writeBlock) {
        ...
        if (values.contains(m -> annotation(m, Combine.class) != null)) {
            ...
            for (ExecutableElement elem : values) {
                ...
                if (is(elem, Modifier.ABSTRACT) || is(elem, Modifier.NATIVE) || (!methodBlocks.containsKey(descStr) && insertComp.isEmpty()))
                    continue;
    ...
    public Set<String> getSupportedAnnotationTypes() {
        return Set.of(EntityComponent.class.getCanonicalName(), EntityBaseComponent.class.getCanonicalName(), EntityDef.class.getCanonicalName(), EntityPoint.class.getCanonicalName());
    }
    ...
    private static class EntityDefinition {
        ...
        EntityDefinition(String string, TypeSpec.Builder build, Element element, TypeName ext, Seq<TypeElement> component, Seq<String> group, Seq<FieldSpec> field) {
            builder = build;
            name = string;
            naming = element;
            groups = group;
