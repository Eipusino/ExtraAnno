...
import heavyindustry.annotations.Annotations.Resolve.*;
...
@SupportedAnnotationTypes({
    "heavyindustry.annotations.Annotations.EntityComponent",
    "heavyindustry.annotations.Annotations.EntityBaseComponent",
    "heavyindustry.annotations.Annotations.EntityDef",
    "heavyindustry.annotations.Annotations.EntityPoint"
})
public class EntityProcessor extends BaseProcessor {
    ...
    public void process(RoundEnvironment roundEnv) throws Exception{
        ...
        if(round == 1){
            ...
            TypeSpec.Builder map = TypeSpec.classBuilder("HIEntityMapping").addModifiers(Modifier.PUBLIC)
                .addField(
                ...
                .addMethod(
                    MethodSpec.methodBuilder("register")
                        ...
                        .beginControlFlow("synchronized($T.class)", ClassName.get(packageName, "HIEntityMapping"))
            ...
            for(EntityDefinition def : definitions){
                ...
                def.builder.addMethod(
                    MethodSpec.methodBuilder("classId").addModifiers(Modifier.PUBLIC)
                        .addAnnotation(cName(Override.class))
                        .returns(TypeName.INT)
                        .addStatement("return $T.classId($T.class)", ClassName.get(packageName, "HIEntityMapping"), type)
                    .build()
                );
            ...
            for(EntityDefinition def : definitions){
                ...
                for(TypeElement comp : def.components){
                    ...
                    for(ExecutableElement method : methods(inter)){
                        ...
                        if(method.getReturnType().getKind() == VOID && !Seq.with(field.annotations).contains(f -> f.type.toString().equals("@heavyindustry.annotations.Annotations.ReadOnly"))){
